    ...
    pub(crate) fn compile() -> Result<(), Box<dyn Error>> {
        ...
        for (proto, out) in grpc_spec_files.iter().zip(&out_files) {
            let build_client = (cfg!(feature = "client")
                ...
                || (cfg!(feature = "server") && out.ends_with("callback.rs"));
            let build_server = (cfg!(feature = "client")
                && out.ends_with("callback.rs"))
                || (cfg!(feature = "server") && out.ends_with("api.rs"));
        ...
        fs::remove_file(format!("{}/google.protobuf.rs", GRPC_DIR)).map_err(
            |e| {
                format!(
                    "`google.protobuf.rs` file isn't generated. \
                     This is good news, because hyperium/tonic#314 \
                     issue was probably really fixed. Check it and \
                     if so, then just remove this line of code.\n\
                     Error: {}",
                    e,
                )
            },
