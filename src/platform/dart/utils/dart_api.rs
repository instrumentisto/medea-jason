...
extern "C" {
    ...
    /// object pointed to by the handle is garbage collected, in this case
    /// the VM automatically deletes the handle after invoking the callback
    /// associated with the handle.
    ///
    /// When finalizable handle will be collected by GC, then provided
    /// `callback` will be called.
    ///
    /// `peer` argument will be provided to the `callback` on finalize.
    ///
    /// `external_allocation_size` is size of the `peer` which cann be
    /// calculated with [`mem::size_of`].
    ///
